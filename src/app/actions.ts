...
type UpdateStatusPayload = {
  booking_id: string;
  status: 'assigned' | 'accepted' | 'job_rejected' | 'on_the_way' | 'in-progress' | 'code_sent' | 'completed' | 'cancelled' | 'inspection_started' | 'inspection_completed' | 'quotation_shared' | 'quotation_approved' | 'quotation_rejected' | 'repair_started' | 'repair_completed' | 'closed_no_repair' | 'payment_pending';
  note: string;
  order_id: string;
  final_cost?: number;
...
export async function getJobsAction() {
  const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/booking?select=final_amount_to_be_paid%2Cfinal_amount_paid%2Cnet_inspection_fee%2Cuser_name%2Cid%2Corder_id%2Cstatus%2Ccreated_at%2Cfull_address%2Cpreferred_time_slot%2Cpreferred_service_date%2Cpincode%2Ctotal_estimated_price%2Cmedia_url%2Cmobile_number%2Cmap_url%2Ccategories(id%2Cname)%2Cissues(id%2Ctitle)&order=created_at.desc`;
  const apikey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  if (!url || !apikey) {
    console.error("Supabase URL or anon key is not defined in environment variables.");
    throw new Error("Server configuration error.");
...
export async function getTechnicianStats(): Promise<TechnicianStats | null> {
  const url = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/technician_stats`;
  const apikey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
